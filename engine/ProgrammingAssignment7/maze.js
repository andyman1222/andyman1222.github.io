...
var materials = [new _Material(1, [vec4(1,0,0,1), vec4(0,0,0,1), vec4(1,1,1,1), vec4(1,1,1,1), vec4(0,1,1,1), vec4(1,1,0,0)]),
  new _Material(1, [vec4(0,1,0,1), vec4(1,1,1,1), vec4(0,0,0,1), vec4(1,1,1,1), vec4(1,1,1,1), vec4(1,1,0,0)]),
  new _Material(1, [vec4(0,0,1,1), vec4(1,1,1,1), vec4(1,1,1,1), vec4(0,0,0,1), vec4(10,1,1,1), vec4(1,1,0,0)]),
  new _Material(1, [vec4(1,1,0,1), vec4(0,0,0,1), vec4(0,0,0,1), vec4(1,1,1,1), vec4(1,1,1,1), vec4(1,1,0,0)]),
  new _Material(1, [vec4(1,0,1,1), vec4(1,1,1,1), vec4(1,1,1,1), vec4(1,1,1,1), vec4(100,1,1,1), vec4(1,1,0,0)]),
  new _Material(1, [vec4(0,1,1,1), vec4(0,0,0,1), vec4(0,0,0,1), vec4(1,1,1,1), vec4(1,1,1,1), vec4(1,1,0,0)])]
  ...
  /* draw the edges as lines */
  {
    ...
    for (i=0; i<perimeters; i++) {
      if (perimeter[i].draw == true) {
        ...
        tmp.points, [new _ScaledTexMat(false, .001, .001)], _Bounds._RECT, [new _ComplexTexture(_gl, ["images/Brick_Wall_015_COLOR.jpg", "images/Brick_Wall_015_DISP.png", "images/Brick_Wall_015_NORM.jpg", "images/Brick_Wall_015_OCC.jpg", "images/Brick_Wall_015_ROUGH.jpg"])]))
      }
    }
  }
}
