...
void main(void){
    ...
    fColor = mix(fogColor, texture(scene, texCoords), clamp(((texture(depth, texCoords)+minViewDist)*(maxViewDist-minViewDist))*((texture(position, texCoords).y+minViewHeight)*(maxViewHeight-minViewHeight)),0.,1.));
