...
void main(void){
    ...
    fColor = mix(texture(scene, texCoords), fogColor, 1.-clamp(((texture(depth, texCoords).r-minViewDist)*(maxViewDist-minViewDist))+((texture(position, texCoords).y+minViewHeight)*(maxViewHeight-minViewHeight)),0.,1.));
