	...
	_init(shaderProgram, postShaderProgram) {
		...
		if (this._setupInfo.postTexStr != null) {
			...
			for (var i = 0; i < this._setupInfo.postTexCount; i++) {
				...
				this._gTarget.texImage2D(this._gTarget.TEXTURE_2D, 0, (_FLOATING_EXT && _FLOATING_BUF_EXT ? this._gTarget.RGBA32F : this._gTarget.RGBA), this._gTarget.canvas.clientWidth, this._gTarget.canvas.clientHeight, 0,
					this._gTarget.RGBA, (_FLOATING_EXT && _FLOATING_BUF_EXT ? this._gTarget.FLOAT : this._gTarget.UNSIGNED_BYTE), null); //all postprocess textures will support floating point if possible
