...
sMat getStandardLight(vec4 mp5, vec3 norm, vec3 pos, vec3 viewPos){
	...
	for(int x=0;x<=maxLightIndex;x++){
		switch(lights[x].type){
			...
			case 3://point
			vec3 v_surfaceToLight=((TBN*(lights[x].location))-position)*vec3(1,1,-1); //potentially expensive operation and repetitive per vertex but I can't figure out how to calculate it in vertex
