...
sMat getStandardLight(vec4 mp5, vec3 norm, vec3 pos, vec3 viewPos){
	...
	for(int x=0;x<=maxLightIndex;x++){
		switch(lights[x].type){
			...
			case 3://point
			vec3 v_surfaceToLight=((TBN*(lights[x].location*vec3(1.,1.,-1.)))-position); //potentially expensive operation and repetitive per vertex but I can't figure out how to calculate it in vertex
...
vec4 standardImage(vec4 mp[MAT_PROP_COUNT], vec3 pos, vec2 tx, vec3 viewPos){
	vec3 norm = vec3(-1,-1,1)*(TBN*(texture(normalMap, tx).rgb*2.-1.));
	sMat mat = getStandardLight(mp[4], norm, pos, viewPos);
