...
vec2 parallax(vec2 tx, vec3 viewDir, vec3 norm, float minLayers, float maxLayers, float heightScale)
{
	...
	for(float i = 0.; currentDepthMapValue > currentLayerDepth && i < maxLayers; i++)
    {
...
switch(matIndex){
	...
	txc = parallax(txc, -normalize(cameraPosT-positionT)*vec3(1,1,-1), normalT, matProp[4][1], matProp[4][2], matProp[4][3]);
	//break;
