	...
	switch(matIndex){
		...
		for(int x=0;x<=maxLightIndex;x++){
			switch(lights[x].type){
				...
				vec4 tmpDiff=(1-(1./(length(L)*(1./lights[x].attenuation))))*(lights[x].color*lights[x].diffuseMultiply);
				vec4 tmpSpec=(1-(1./(length(L)*(1./lights[x].attenuation))))*(Ks*lights[x].color*lights[x].specularMultiply);
				if(light<0.){
